...
uint8_t I2C::write(uint8_t address, uint8_t registerAddress, uint8_t data)
{
  ...
  return sendData(address, buffer, 2);
}
...
uint8_t I2C::sendData(uint8_t address, const uint8_t *data, uint8_t length)
{
  returnStatus = _start();
  if (returnStatus)
    return returnStatus;
...
uint8_t I2C::receiveData(uint8_t address, uint8_t *buffer, uint8_t length)
{
  bytesAvailable = 0;
  bufferIndex = 0;
  nack = length - 1;
  ...
  for (uint8_t i = 0; i < length; ++i)
  {
    returnStatus = _receiveByte(i == nack ? 0 : 1);
    if (returnStatus)
      return returnStatus;
    buffer[i] = TWDR;
    bytesAvailable = i + 1;
