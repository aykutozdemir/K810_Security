...
void MemoryUsage::ramDisplay(Print & out)
{
	...
	out.println(F("********************")); 
	out.print(F("+----------------+ "));		out.print((int)&__data_start);	out.println(F(" (__data_start)"));
	out.print(F("+      data      +"));			out.println();
	out.print(F("+    variables   + size = "));	out.println(data_size);
	out.print(F("+----------------+ "));		out.print((int)&__data_end);	out.println(F(" (__data_end / __bss_start)"));
	out.print(F("+      bss       +"));			out.println();
	out.print(F("+    variables   + size = "));	out.println(bss_size);
	out.print(F("+----------------+ "));		out.print((int)&__bss_end);		out.println(F(" (__bss_end / __heap_start)"));
	out.print(F("+      heap      + size = "));	out.println(heap_size);
	out.print(F("+----------------+ "));		out.print((int)heap_end);		out.println(F(" (__brkval if not 0, or __heap_start)"));
	out.print(F("+                +"));			out.println();
	out.print(F("+                +"));			out.println();
	out.print(F("+   FREE RAM     + size = "));	out.print(available);			out.print(F(", min_size = "));	out.println(min_available);
	out.print(F("+                +"));			out.println();
	out.print(F("+                +"));			out.println();
	out.print(F("+----------------+ "));		out.print((int)SP);				out.println(F(" (SP)"));
	out.print(F("+     stack      + size = "));	out.println(stack_size);
	out.print(F("+----------------+ "));		out.print((int)RAMEND);			out.println(F(" (RAMEND / __stack)"));
	out.println(F("********************")); 
	...
	if (min_available < 100)
	{
		...
		out.print(F("Warning: Possible collision between heap and stack: %"));	out.println(100 - j);
		out.println(F("********************")); 
	}
}